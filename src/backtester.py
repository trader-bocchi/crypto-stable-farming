...
class Backtester:
    ...
    def run(self) -> dict:
        ...
        print(f"  그리드 수: {self.config.grid_count}개")
        ...
        for ts, row in self.merged_df.iterrows():
            usdkrw = float(row["usdkrw"]) if not np.isnan(row["usdkrw"]) else None
        ...
        self._portfolio_series = pd.Series(portfolio_values, index=self.merged_df.index, name="portfolio")
    ...
    def print_summary(self, results: dict) -> None:
        ...
        print(f"\n{'═' * W}")
        print(f"  USDT/KRW 그리드 매매 백테스트 결과")
        print(f"  기간: {self.start_date} ~ {self.end_date}")
    ...
    def _plot_main(self, results: dict) -> None:
        ...
        kp = self.merged_df["kimchi_premium"].dropna()
        if not kp.empty:
            ax2.plot(kp.index, kp, color="#6A1B9A", lw=1.5)
            ax2.axhline(0, color="black", ls="-", alpha=0.4)
            ax2.axhline(kp.mean(), color="red", ls="--", alpha=0.7,
                        label=f"평균 {kp.mean():.2f}%")
            ax2.fill_between(kp.index, kp, 0, where=(kp >= 0),
                             color="#4CAF50", alpha=0.3, label="프리미엄")
            ax2.fill_between(kp.index, kp, 0, where=(kp < 0),
                             color="#F44336", alpha=0.3, label="디스카운트")
            ax2.set_ylabel("김치프리미엄 (%)")
            ax2.legend(fontsize=9)
        else:
            ax2.text(0.5, 0.5, "USD/KRW 데이터 없음",
                     transform=ax2.transAxes, ha="center", va="center", color="gray")
        ax2.set_title("USDT 김치프리미엄 (USDT/KRW vs USD/KRW 환율)")
        ...
        ax3.fill_between(drawdown.index, drawdown, 0, color="#F44336", alpha=0.6, label="낙폭")
        ax3.set_ylabel("낙폭 (%)")
        ax3.set_title("포트폴리오 낙폭 (Drawdown)")
